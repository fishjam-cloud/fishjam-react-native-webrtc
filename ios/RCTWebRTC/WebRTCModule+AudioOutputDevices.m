...
- (void)ensureAudioRouteObserver {
    if (objc_getAssociatedObject(self, AudioRouteObserverKey)) return;
...
- (void)handleAudioRouteChange:(NSNotification *)notification {
    ...
    [self sendEventWithName:kEventAudioOutputChanged body:@{
        @"reason": @(reason),
...
- (NSDictionary *)routeToDict:(AVAudioSessionRouteDescription *)route {
    ...
    for (AVAudioSessionPortDescription *port in route.inputs) {
        [inputs addObject:@{
            @"type": [self portTypeToString:port.portType],
            @"name": port.portName
        }];
    }
...
RCT_EXPORT_METHOD(overrideAudioOutput:(NSString *)output
    ...
    if ([output isEqualToString:@"speaker"]) {
        ...
        reject(@"E_AUDIO_OUTPUT_INVALID",
               [NSString stringWithFormat:@"Invalid output '%@', expected 'speaker' or 'none'", output], nil);
        return;
    }
...
RCT_EXPORT_METHOD(getCurrentAudioOutput:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject) {
    [self ensureAudioRouteObserver];
